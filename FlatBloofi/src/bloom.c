...
{
  return bloom_init(bloom, entries, error,metric,hash);
}
...
int bloom_init(struct bloom * bloom, int entries, double error,int metric,struct hasher* hash)
{
  ...
  num=-(num/0.6931471805599453);
  num=ceil(num);
 // double denom = 0.480453013918201; // ln(2)^2
  double denom=0.6931471805599453;
  bloom->bpe = (num / denom);
 ...
 // bloom->bf = (unsigned char *)calloc(bloom->bytes, sizeof(unsigned char));
  bloom->h=hash;
  bloom->h->maxval=bloom->bits;
  setNumberOfRandomKeys(bloom->h,bloom->hashes);
